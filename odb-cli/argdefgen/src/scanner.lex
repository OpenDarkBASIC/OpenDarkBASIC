...
%%
<INITIAL,HELP>{
    ^" "+?section" "{SEC_NAME}:      { BEGIN(INITIAL); yylval->string_value = section_name(yytext); return TOK_SECTION; }
    ^" "+?func:" "+?{FUNC_NAME}      { BEGIN(INITIAL); yylval->string_value = func_name(yytext); return TOK_FUNC; }
    ...
    ^" "+?{IMPLICIT_ACTION}:         { BEGIN(INITIAL); yylval->string_value = action_name(yytext); return TOK_IMPLICIT_ACTION; }
}
<INITIAL>{
