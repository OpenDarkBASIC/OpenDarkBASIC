...
TEST_F(NAME, push_with_expand_sets_count_and_capacity_correctly)
{
    ...
    EXPECT_THAT(
        vobj.mem->capacity,
        Eq(ODBSDK_VEC_MIN_CAPACITY * ODBSDK_VEC_EXPAND_FACTOR));
}
...
TEST_F(NAME, push_with_expand_has_correct_values)
{
    ...
    EXPECT_THAT(
        (vec_get(vobj, ODBSDK_VEC_MIN_CAPACITY)), Pointee(obj{42, 42, 42, 42}));
}
...
TEST_F(NAME, insert_with_expand_has_correct_values)
{
    ...
    for (uint64_t i = 0; i != 3; ++i)
        EXPECT_THAT(
            (vec_get(vobj, i)),
            Pointee(
                obj{ODBSDK_VEC_MIN_CAPACITY - i - 1,
                    ODBSDK_VEC_MIN_CAPACITY - i - 1,
                    ODBSDK_VEC_MIN_CAPACITY - i - 1,
                    ODBSDK_VEC_MIN_CAPACITY - i - 1}));
    ...
    for (uint64_t i = 4; i != ODBSDK_VEC_MIN_CAPACITY + 1; ++i)
        EXPECT_THAT(
            (vec_get(vobj, i)),
            Pointee(
                obj{ODBSDK_VEC_MIN_CAPACITY - i,
                    ODBSDK_VEC_MIN_CAPACITY - i,
                    ODBSDK_VEC_MIN_CAPACITY - i,
                    ODBSDK_VEC_MIN_CAPACITY - i}));
}
