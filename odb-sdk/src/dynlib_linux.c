...
static ElfW(Addr) get_symbol_count_in_hash_table(const uint32_t* hashtab)
{
...
static ElfW(Addr) get_symbol_count_in_GNU_hash_table(const uint32_t* hashtab)
{
    ...
    void*          user)
{
    ...
    if (dlinfo((void*)handle, RTLD_DI_LINKMAP, &lm) != 0)
        return -1;
    ...
    symcount = hashtab ? get_symbol_count_in_hash_table(hashtab)
