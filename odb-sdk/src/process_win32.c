    ...
    struct utf8*      err)
{
    ...
    if (!ret)
    {
        log_sdk_err("Failed to create process {quote:%s}: {win32error}\n", cmdline);
    ...
    if (!GetExitCodeProcess(pi.hProcess, &exitcode))
    {
        ...
        return log_sdk_err("Failed to get exit code from process: {win32error}\n");
