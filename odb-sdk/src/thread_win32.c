...
thread_start(struct thread* t, void* (*func)(const void*), const void* args)
{
    ...
    if (hThread == NULL)
    {
        log_sdk_err("Failed to create thread: {win32error}\n");
        return -1;
    }
...
thread_join(struct thread t, int timeout_ms)
{
    ...
    {
    case WAIT_FAILED:
        log_sdk_err("WaitForSingleObject failed in thread_join(): {win32error}\n");
        /* fallthrough */
    case WAIT_ABANDONED:
    case WAIT_TIMEOUT:
        return -1;
...
thread_kill(struct thread t)
{
    ...
    if (TerminateThread(hThread, (DWORD)-1) == FALSE)
        log_sdk_err("Failed to TerminateThread: {win32error}\n");
    CloseHandle(hThread);
}
