...
TEST(NAME, insert_new_with_realloc_shifts_data_correctly)
{
    ...
    btree_size midway = ODBUTIL_BTREE_MIN_CAPACITY / 2;

    int value = 0x55;
    btree_reserve(&btree, ODBUTIL_BTREE_MIN_CAPACITY);
    for (int i = 0; i != ODBUTIL_BTREE_MIN_CAPACITY; ++i)
    {
        if (i < midway)
    ...
    ASSERT_THAT(btree_capacity(&btree), Eq(ODBUTIL_BTREE_MIN_CAPACITY));
    ASSERT_THAT(btree_insert_new(&btree, midway, &value), Eq(1));
    // Now it should have reallocated
    ASSERT_THAT(btree_capacity(&btree), Gt(ODBUTIL_BTREE_MIN_CAPACITY));

    // Check all values are there
    for (int i = 0; i != ODBUTIL_BTREE_MIN_CAPACITY+1; ++i)
...
TEST(NAME, compact_reduces_capacity_and_keeps_elements_in_tact)
{
    ...
    int a = 53;
    for (int i = 0; i != ODBUTIL_BTREE_MIN_CAPACITY * 3; ++i)
    ...
    EXPECT_THAT(btree_count(&btree), Eq(ODBUTIL_BTREE_MIN_CAPACITY * 2));
    EXPECT_THAT(btree_capacity(&btree), Eq(ODBUTIL_BTREE_MIN_CAPACITY * 2));
    EXPECT_THAT(btree.data, NotNull());
