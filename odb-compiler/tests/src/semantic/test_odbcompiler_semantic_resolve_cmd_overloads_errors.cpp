...
TEST_F(NAME, ambiguous_overloads_1)
{
    ...
    EXPECT_THAT(
        log(),
        LogEq("test:1:7\n"
              "error: Command has ambiguous overloads. Unable to match "
              "argument 2 to command signature.\n"
              " 1 | print 5, 6\n"
              "   |          ^ BYTE\n"
...
TEST_F(NAME, ambiguous_overloads_2)
{
    ...
    EXPECT_THAT(
        ...
        LogEq("test:1:7\n"
              "error: Command has ambiguous overloads. Unable to match "
              "arguments 1 and 2 to command signature.\n"
              " 1 | print 5, 6\n"
              "   |       ^  ^ BYTE\n"
              "   |       BYTE\n"
              "note: Conflicting overloads are:\n"
...
TEST_F(NAME, ambiguous_overloads_no_params)
{
    ...
    EXPECT_THAT(
        log(),
        LogEq("test:1:1\n"
              "error: Command has multiple definitions.\n"
              " 1 | print\n"
              "   | ^~~~<\n"
...
TEST_F(NAME, multi_arg_ambiguous_overloads)
{
    ...
    EXPECT_THAT(
        ...
        /* clang-format off */
        LogEq("test:1:7\n"
              "error: Command has ambiguous overloads. Unable to match arguments 2, 3 and 4 to command signature.\n"
              " 1 | print 5, 6, 7, 8, 9\n"
              "   |          ^  ^  ^ BYTE\n"
              "   |          |  BYTE\n"
              "   |          BYTE\n"
...
TEST_F(NAME, dont_highlight_brackets_in_expr)
{
    ...
    ASSERT_THAT(
        log(),
        LogEq("test:1:7\n"
              "error: Parameter mismatch: No version of this command takes the "
