...
TEST_F(NAME, value_of_0_and_1_is_type_byte_and_not_boolean)
{
    ...
    ast_id cmd1 = ast.nodes[0].block.stmt;
    ast_id cmd2 = ast.nodes[ast.nodes[0].block.next].block.stmt;
...
TEST_F(NAME, double_integer_literal)
{
    ...
    ast_id lit1 = ast.nodes[arg1].arglist.expr;
    ast_id lit2 = ast.nodes[arg2].arglist.expr;
    EXPECT_THAT(ast.nodes[lit1].info.node_type, Eq(AST_DOUBLE_INTEGER_LITERAL));
    EXPECT_THAT(ast.nodes[lit2].info.node_type, Eq(AST_DOUBLE_INTEGER_LITERAL));
    EXPECT_THAT(ast.nodes[lit1].double_integer_literal.value, Eq(4294967296u));
    EXPECT_THAT(ast.nodes[lit2].double_integer_literal.value, Eq(9223372036854775807u));
}
...
TEST_F(NAME, hex_literals)
{
    ...
    ast_id cmdbyte = ast.nodes[0].block.stmt;
