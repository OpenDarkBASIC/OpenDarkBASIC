    ...
    const struct candidates*  candidates)
{
    ...
    {
        struct utf8_view name = utf8_list_view(cmds->cmd_names, *cmdp);
        const struct cmd_param_types_list* param_types
            = cmds->param_types->data[*cmdp];
        struct utf8_list*         param_names = cmds->param_names->data[*cmdp];
        union type                ret_type = cmds->return_types->data[*cmdp];
    ...
    cmd_id                    cmd)
{
    ...
    cmd_name = utf8_list_view(cmds->cmd_names, cmd);
    for (; cmd < cmd_list_count(cmds)
           && utf8_equal(cmd_name, utf8_list_view(cmds->cmd_names, cmd));
         ++cmd)
    {
        int                       i;
        union type                ret_type = cmds->return_types->data[cmd];
        plugin_id                 plugin_id = cmds->plugin_ids->data[cmd];
        const struct plugin_info* plugin = &plugins->data[plugin_id];
        const struct cmd_param_types_list* param_types
            = cmds->param_types->data[cmd];
        struct utf8_list* param_names = cmds->param_names->data[cmd];
        log_raw(
            "%*s|   {emph:%.*s}%s",
            gutter,
            "",
    ...
    const struct udt_storage* udts)
{
         ...
         ++i, arglist = ast->nodes[arglist].arglist.next)
    {
        ...
        if (!types_equal(arg_type, param_type))
        {
            ...
            ast->nodes[as].info.type_info = param_type;
    ...
    struct candidates** candidates, const struct cmd_list* cmds, cmd_id cmd_id)
{
    ...
    struct utf8_view cmd_name = utf8_list_view(cmds->cmd_names, cmd_id);
    do
    {
        if (candidates_push(candidates, cmd_id++) != 0)
            return -1;
    } while (cmd_id < cmd_list_count(cmds)
             && utf8_equal(cmd_name, utf8_list_view(cmds->cmd_names, cmd_id)));
