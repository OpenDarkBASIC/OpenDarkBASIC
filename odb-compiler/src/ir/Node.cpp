...
Type Type::getUDT(UDTDefinition* udt) {
...
{
    return isUDT() ? std::optional<UDTDefinition*>{std::get_if<UDTType>(&variant_)->udt} : std::nullopt;
}
...
std::string Type::toString() const
{
    if (isBuiltinType())
    {
        return convertBuiltinTypeToString(*std::get_if<BuiltinType>(&variant_));
    }
    else if (isArray())
    {
