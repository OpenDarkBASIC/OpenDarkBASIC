...
public:
    ...
    public:
        ...
        llvm::Function* getOrCreateKeywordThunk(const Keyword* keyword, int keywordOverloadIndex);
    ...
    llvm::Value* generateCast(llvm::IRBuilder<>& builder, llvm::Value* expression,
...
llvm::Function* CodeGenerator::GlobalSymbolTable::getOrCreateKeywordThunk(const Keyword* keyword,
                                                                          int keywordOverloadIndex)
{
    ...
    argTypes.reserve(keywordOverload.arglist.size());
    for (const auto& arg : keywordOverload.arglist)
    {
    ...
    {
        returnTy = getLLVMType(module.getContext(), *keyword->returnType);
    }
    else
    {
    ...
    llvm::Function* function = engineInterface.generateCommandCall(
        *keyword, keywordOverload, "DBCommand" + commandName, functionTy);
...
llvm::AllocaInst* CodeGenerator::SymbolTable::getOrAddVar(const std::string& name, llvm::Type* type)
{
...
llvm::Value* CodeGenerator::SymbolTable::getOrAddStrLiteral(const std::string& literal)
{
    ...
    llvm::Value* literalStorage =
        new llvm::GlobalVariable(getGlobalTable().getModule(), stringConstant->getType(), false,
...
llvm::Value* CodeGenerator::generateCast(llvm::IRBuilder<>& builder, llvm::Value* expression,
                                         llvm::Type* targetType)
{
...
llvm::Function* CodeGenerator::generateFunction(FunctionDefinition* f)
{
    ...
    {
        functionName = f->name;
        returnTy = getLLVMType(ctx, f->returnType);
    }
                                               ...
                                               Expression* e)
{
        ...
        case BinaryOp::Add:
            ...
            else if (left->getType()->isFloatTy())
            {
                return builder.CreateFAdd(left, right);
                     ...
                     left->getType()->getPointerElementType()->isIntegerTy(8))
            {
                // TODO: add string.
    ...
    {
        llvm::AllocaInst* variableInst =
            symtab.getOrAddVar(varRef->name, getLLVMType(ctx, varRef->type));
    ...
    else if (auto* literal = dynamic_cast<LiteralExpression*>(e))
    {
        ...
        case ast::LT_STRING: {
            return builder.CreateBitCast(symtab.getOrAddStrLiteral(literal->value.s),
...
llvm::BasicBlock* CodeGenerator::generateBlock(SymbolTable& symtab, const StatementBlock& block,
                                               llvm::Function* function, const std::string& name)
{
    ...
    for (const auto& statement_ptr : block)
    {
        ...
        else if (auto* branch = dynamic_cast<BranchStatement*>(s))
        {
            ...
            llvm::BasicBlock* trueBlock = generateBlock(symtab, branch->trueBranch, function, "if");
            llvm::BasicBlock* falseBlock =
                generateBlock(symtab, branch->falseBranch, function, "else");
...
void CodeGenerator::generateModule(const Program& program, std::vector<std::string> pluginsToLoad)
{
    ...
    // Generate blocks.
    llvm::IRBuilder<> builder(ctx);
    builder.SetInsertPoint(generateBlock(*symbolTables[gameEntryPointFunc], program.mainStatements,
                                         gameEntryPointFunc, "entry"));
...
void generateLLVMIR(std::ostream& os, const std::string& moduleName, Program& program,
                    const KeywordIndex& kwIndex)
{
    ...
    gen.generateModule(program, kwIndex.pluginsAsList());
...
void generateLLVMBC(std::ostream& os, const std::string& moduleName, Program& program,
                    const KeywordIndex& kwIndex)
{
    llvm::LLVMContext context;
    llvm::Module module(moduleName, context);
...
void generateObjectFile(std::ostream& os, const std::string& moduleName, Program& program,
                        const KeywordIndex& kwIndex)
{
    llvm::LLVMContext context;
    llvm::Module module(moduleName, context);
...
void generateExecutable(std::ostream& os, const std::string& moduleName, Program& program,
                        const KeywordIndex& kwIndex)
{
    llvm::LLVMContext context;
    llvm::Module module(moduleName, context);
