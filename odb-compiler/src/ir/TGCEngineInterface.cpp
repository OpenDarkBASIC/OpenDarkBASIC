...
TGCEngineInterface::TGCEngineInterface(llvm::Module& module) : EngineInterface(module)
{
    ...
    loadLibraryFunc =
        llvm::Function::Create(llvm::FunctionType::get(hInstanceTy, {stringTy}, false),
    ...
    getLastErrorFunc =
        llvm::Function::Create(llvm::FunctionType::get(dwordTy, {}, false),
    ...
    getProcAddrFunc =
        llvm::Function::Create(llvm::FunctionType::get(procAddrTy, {hInstanceTy, stringTy}, false),
...
llvm::Function* TGCEngineInterface::generateCommandCall(const Keyword& keyword,
                                                        const Keyword::Overload& overload,
                                                        ...
                                                        llvm::FunctionType* functionType)
{
    ...
    llvm::FunctionType* pluginFunctionType = llvm::FunctionType::get(pluginReturnType, functionType->params(), functionType->isVarArg());

    // Obtain function ptr from the relevant plugin.
    // TODO: Call this once at the beginning of the application.
    auto commandFunction = getPluginFunction(builder, pluginFunctionType, keyword.plugin,
                                            ...
                                            std::vector<std::string> pluginsToLoad)
{
    ...
    llvm::Function* entryPointFunc =
        llvm::Function::Create(llvm::FunctionType::get(llvm::Type::getInt32Ty(ctx), {}),
    ...
    {
        pluginLoadingBlocks.emplace_back(
            llvm::BasicBlock::Create(ctx, "load" + getPluginName(plugin), entryPointFunc));
    }
    llvm::BasicBlock* failedToLoadPluginsBlock =
        llvm::BasicBlock::Create(ctx, "failedToLoadPlugin", entryPointFunc);
    llvm::BasicBlock* initialiseEngineBlock =
        llvm::BasicBlock::Create(ctx, "initialiseEngine", entryPointFunc);
    ...
    for (int i = 0; i < pluginsToLoad.size(); ++i)
    {
        ...
        printString(builder, loadingPluginString);
        printString(builder, pluginNameConstant);

        // Check if loaded successfully.
        auto* nextBlock =
            i == (pluginsToLoad.size() - 1) ? initialiseEngineBlock : pluginLoadingBlocks[i + 1];
    ...
    printString(builder, builder.CreateGlobalStringPtr(
                             "Failed to load plugin. GetLastError returned"));
    ...
    auto getGlobStructFunc = getPluginFunction(builder, llvm::FunctionType::get(dwordTy, false),
                                               "DBProCore.dll", "?GetGlobPtr@@YAKXZ");
    ...
    for (const auto& [plugin, globStructIndex] : pluginGlobStructIndices)
    {
        ...
        gepIndices.emplace_back(
            llvm::ConstantInt::get(llvm::Type::getInt32Ty(ctx), globStructIndex + 1));
    ...
    auto passDLLsFunc =
        getPluginFunction(builder, llvm::FunctionType::get(llvm::Type::getVoidTy(ctx), false),
                          "DBProCore.dll", "?PassDLLs@@YAXXZ");
    auto initDisplayFunc = getPluginFunction(
        builder,
        llvm::FunctionType::get(dwordTy,
                                {dwordTy, dwordTy, dwordTy, dwordTy, hInstanceTy, stringTy}, false),
        "DBProCore.dll", "?InitDisplay@@YAKKKKKPAUHINSTANCE__@@PAD@Z");
    auto closeDisplayFunc = getPluginFunction(builder, llvm::FunctionType::get(dwordTy, false),
    ...
    llvm::BasicBlock* failedToInitDisplayBlock =
        llvm::BasicBlock::Create(ctx, "failedToInitDisplay", entryPointFunc);
    llvm::BasicBlock* launchGameBlock = llvm::BasicBlock::Create(ctx, "launchGame", entryPointFunc);

    builder.CreateCondBr(
        builder.CreateICmpEQ(initDisplayResult, llvm::ConstantInt::get(dwordTy, 0)),
        launchGameBlock, failedToInitDisplayBlock);

    builder.SetInsertPoint(failedToInitDisplayBlock);
    printString(builder,
                builder.CreateGlobalStringPtr("Failed to init display."));
    builder.CreateRet(llvm::ConstantInt::get(llvm::Type::getInt32Ty(ctx), 1));
...
llvm::FunctionCallee TGCEngineInterface::getPluginFunction(llvm::IRBuilder<>& builder,
                                                           ...
                                                           const std::string& symbolStringName)
{
    ...
    llvm::CallInst* procAddress = builder.CreateCall(getProcAddrFunc, {pluginHModule, builder.CreateGlobalStringPtr(symbol, symbolStringName)});
...
void TGCEngineInterface::printString(llvm::IRBuilder<>& builder, llvm::Value* string)
{
    ...
    if (!putsFunc)
    {
        putsFunc = llvm::Function::Create(
            llvm::FunctionType::get(llvm::Type::getInt32Ty(ctx), {stringTy}, false),
...
llvm::Value* TGCEngineInterface::convertIntegerToString(llvm::IRBuilder<> &builder, llvm::Value *integer)
{
    ...
    if (!ltoaFunc)
    {
      ltoaFunc = llvm::Function::Create(
          llvm::FunctionType::get(
              stringTy, {llvm::Type::getInt32Ty(ctx), stringTy, llvm::Type::getInt32Ty(ctx)},
              false),
    ...
    auto* buffer = builder.CreateAlloca(
        llvm::Type::getInt8Ty(ctx), llvm::ConstantInt::get(llvm::Type::getInt32Ty(ctx), 20));
    builder.CreateCall(
