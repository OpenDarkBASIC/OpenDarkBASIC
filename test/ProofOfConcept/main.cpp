...
struct GlobStruct2
{
	...
	HINSTANCE					g_Spare18			;
	HINSTANCE					g_Spare19			;
	HINSTANCE					g_Spare20			; // 16 x 4 
	bool						g_GFXmade			; // 53 x 4
	bool						g_Textmade			;
	bool						g_Basic2Dmade		;
	bool						g_Spritesmade		;
	bool						g_Imagemade			;
...
int main() {
    std::cout << (offsetof(GlobStruct2, pEXEUnpackDirectory) - offsetof(GlobStruct2, g_GFXmade)) << std::endl;
    ...
    auto image_dll = plugin_handles["DBProImageDebug.dll"];
    ...
    std::cout << glob_ptr->pEXEUnpackDirectory << std::endl;
...
//    PassCmdLinePtr((void*)cmdline_ptr);
    ...
    auto LoadImage_ = (void (*)(const char*, int))GetProcAddress(image_dll, "?LoadEx@@YAXPADH@Z");
    ...
    auto ObjectAngleY = [&](int i) -> float {
        DWORD value = ((DWORD(*)(int))GetProcAddress(basic3d_dll, "?GetYRotationEx@@YAKH@Z"))(i);
        return *(float*)&value;
    };

    LoadImage_("earth - Copy.bmp", 1);
    MakeObjectCube(1, 10.0f);
    printf("%f\n", ObjectAngleY(1));
    while (true) {
        YRotateObject(1, ObjectAngleY(1) + 0.1f);
        Sync();
    }
